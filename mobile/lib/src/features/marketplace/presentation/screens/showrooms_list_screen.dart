...
import 'package:mobile/src/features/marketplace/presentation/widgets/showroom_card.dart';

class ShowroomsListScreen extends ConsumerStatefulWidget {
  const ShowroomsListScreen({super.key});

  @override
  ConsumerState<ShowroomsListScreen> createState() => _ShowroomsListScreenState();
}

class _ShowroomsListScreenState extends ConsumerState<ShowroomsListScreen> {
  final _search = TextEditingController();
  final _carType = TextEditingController();
  final _model = TextEditingController();
  final _color = TextEditingController();
  int? _provinceId;
  ShowroomSearchFilters? _filters;
  ...
  ShowroomSearchFilters _buildFilters() {
    return ShowroomSearchFilters(
  ...
  void _applyFilters({bool keepKeyboard = false}) {
    ...
    ref.invalidate(showroomsListProvider(next));
  }
  ...
  void _clearFilters() {
    ...
    ref.invalidate(showroomsListProvider(const ShowroomSearchFilters()));
  }

  @override
  Widget build(BuildContext context) {
    final filters = _filters ?? const ShowroomSearchFilters();
    final showrooms = ref.watch(showroomsListProvider(filters));
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
            ...
            child: Column(
              children: [
                TextField(
                  ...
                  decoration: MetallicSilverText.inputDecoration('بحث عن معرض'),
          ...
          SizedBox(height: 10 * scale),
          Expanded(
            child: showrooms.when(
              data: (items) => items.isEmpty
                  ? const Center(child: MetallicSilverText('لا توجد معارض مطابقة'))
                  : RefreshIndicator(
                      onRefresh: () async => ref.invalidate(showroomsListProvider(filters)),
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return ShowroomCard(
                            ...
                            onTap: () => Navigator.push(
                              context,
                              MaterialPageRoute(builder: (_) => ShowroomDetailsScreen(showroomId: s.id)),
                            ),
                          );
                        },
                      ),
